...
const T={
  en:{
    ...
    hLabel:'Beta · English launch',hTitle:'Fight for your brain',
    hSub:'Quick Play, duels, packs and tournaments — all in one place.',
    ...
    wlSub:"Мы строим Mind Fight Club — глобальную платформу для интеллектуальных боёв.",
    wlSubmit:'Get early access →',wlThanksTitle:"You're on the list!",wlThanksSub:"We'll email you when Mind Fight Club launches.",
    ...
    playMenu:'Play',quickPlay:'Быстрая игра',chooseTopic:'Choose Topic',myPacks:'My Packs',
  ...
  ru:{
    ...
    hLabel:'Бета · Запуск',hTitle:'Бой за мозг',
    hSub:'Быстрая игра, дуэли, паки и турниры — всё в одном месте.',
    ...
    wlSub:'Мы строим Mind Fight Club — глобальную платформу для интеллектуальных боёв.',
    wlSubmit:'Получить ранний доступ →',wlThanksTitle:'Вы в списке!',wlThanksSub:'Напишем когда Mind Fight Club запустится.',
    ...
    neuronsExplTitle:'Что такое нейроны?',tracksTitle:'Треки',packsTitle:'Паки вопросов',
...
function applyLang(){
  ...
  setText('nav-teams-label','Команды');
  ...
  setText('pm-packs',L.myPacks||'Паки вопросов');
  ...
  setText('shop-section-title',lang==='ru'?'Паки вопросов':'Question Packs');
  // Profile
...
// ADMIN DASHBOARD
// ═══════════════════════════════════════════
const ADMIN_EMAILS = ['pantandrej@yandex.ru']; // fallback — never remove
...
function startHypePack(packId){
  ...
  if(!qs){ toast('Пак не найден'); return; }
  // Запускаем как обычный квиз из пака
  customQs = qs.map(q => ({ ...q, t: q.a.length * 10 }));
  if(typeof startCustomGame === 'function'){
    startCustomGame(customQs);
  } else {
    // Фолбэк — показываем toast
    toast('🔥 Пак загружен! ' + qs.length + ' вопросов');
  }
}
...
function showPlayMenu(){
  ...
  document.getElementById('pm-packs').textContent = L ? 'Паки вопросов' : 'Question Packs';
...
async function checkLiveOfficialTournament(){
  ...
  if(!item) return;
  try{
    const {data} = await sb.from('official_tournaments').select('code,title').eq('status','live').limit(1).single();
    if(data){
...
// [getTodayKey] → deleted from legacy.js (now in ES module)
...
window.addEventListener('message', (ev)=>{
  if(ev.data?.type === 'vk_auth_success' && ev.data.session){
    sb.auth.setSession(ev.data.session).then(({data})=>{
      if(data.user){ toast('✅ Вход через VK выполнен!'); initAuth(); }
    });
  }
});
...
function renderTeamsScreen(){
  ...
  document.getElementById('t-team-или').textContent = lang==='ru'?'или вступить в существующую':'или вступить в существующую';
  ...
  document.getElementById('nav-teams-label').textContent = 'Команды';
...
function renderShop(){
  ...
  document.getElementById('shop-section-title').textContent = lang==='ru'?'Паки вопросов':'Question Packs';
...
async function submitOrgApplication(){
  ...
  toast('✅ Заявка отправлена! Проверим в течение 1–2 дней.');
