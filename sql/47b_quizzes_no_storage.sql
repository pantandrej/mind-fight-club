-- ══════════════════════════════════════════════════════════════════
-- Migration 47b: Quizzes table + RPCs (без storage — создай бакет вручную)
-- ══════════════════════════════════════════════════════════════════
...
DROP POLICY IF EXISTS "quizzes_owner_read"   ON quizzes;
...
CREATE OR REPLACE FUNCTION create_quiz(
  ...
  p_description text DEFAULT NULL, p_website_url text DEFAULT NULL,
  p_social_links jsonb DEFAULT '{}', p_newcomer_offer text DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_id uuid;
BEGIN
  IF auth.uid() IS NULL THEN RETURN jsonb_build_object('ok',false,'reason','not_authenticated'); END IF;
  IF p_slug !~ '^[a-z0-9\-]{2,50}$' THEN RETURN jsonb_build_object('ok',false,'reason','invalid_slug'); END IF;
  IF EXISTS (SELECT 1 FROM quizzes WHERE slug = p_slug) THEN RETURN jsonb_build_object('ok',false,'reason','slug_taken'); END IF;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM quizzes WHERE id=p_id AND owner_id=auth.uid()) THEN
    RETURN jsonb_build_object('ok',false,'reason','not_owner'); END IF;
  UPDATE quizzes SET name=p_name, description=p_description, website_url=p_website_url,
    social_links=p_social_links, newcomer_offer=p_newcomer_offer,
    logo_url=COALESCE(p_logo_url,logo_url) WHERE id=p_id;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM quizzes WHERE id=p_id AND owner_id=auth.uid()) THEN
    IF NOT EXISTS (SELECT 1 FROM admin_users WHERE user_id=auth.uid() AND is_active=true) THEN
      RETURN jsonb_build_object('ok',false,'reason','forbidden'); END IF; END IF;
...
DECLARE v quizzes;
BEGIN
  SELECT * INTO v FROM quizzes WHERE slug=p_slug AND status='active';
  IF NOT FOUND THEN RETURN jsonb_build_object('ok',false,'reason','not_found'); END IF;
  RETURN jsonb_build_object('ok',true,'quiz',row_to_json(v));
END;$$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM admin_users WHERE user_id=auth.uid() AND is_active=true) THEN
    RETURN jsonb_build_object('ok',false,'reason','not_admin'); END IF;
...
BEGIN
  ...
  RETURN jsonb_build_object('ok',true,'quizzes',(
    ...
    FROM quizzes q WHERE (p_status IS NULL OR q.status=p_status)
  ));
END;$$;
...
CREATE OR REPLACE FUNCTION admin_update_quiz(
  ...
  p_website_url text DEFAULT NULL, p_social_links jsonb DEFAULT NULL,
  p_newcomer_offer text DEFAULT NULL, p_logo_url text DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM admin_users WHERE user_id=auth.uid() AND is_active=true) THEN
    RETURN jsonb_build_object('ok',false,'reason','not_admin'); END IF;
  UPDATE quizzes SET
    ...
    website_url=COALESCE(p_website_url,website_url), social_links=COALESCE(p_social_links,social_links),
    newcomer_offer=COALESCE(p_newcomer_offer,newcomer_offer), logo_url=COALESCE(p_logo_url,logo_url)
  WHERE id=p_id;
  RETURN jsonb_build_object('ok',FOUND);
END;$$;
